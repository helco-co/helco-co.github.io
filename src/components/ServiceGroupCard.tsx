...
export default function ServiceGroupCard({
  ...
  return (
    ...
    <article
      ...
      // h-full so every card in a swipe row matches its neighbours' height
      // rather than each shrinking to its own content.
      className="group h-full scroll-mt-28 rounded-2xl border border-[#30353b] bg-[#1b2025] p-5 transition-all duration-300 max-sm:cursor-pointer hover:z-20 hover:translate-y-[-3px] hover:border-[#a88c68]/60 hover:bg-[#1e242a] hover:shadow-[0_14px_34px_rgba(0,0,0,0.35)] focus-visible:z-20 focus-visible:translate-y-[-3px] focus-visible:border-[#a88c68]/60 focus-visible:bg-[#1e242a] focus-visible:shadow-[0_14px_34px_rgba(0,0,0,0.35)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#a88c68] sm:p-8"
    >
      {/* Icon sits beside the title again: a carousel card is ~281px wide, so
          the stacking that a 166px two-up column forced is no longer needed. */}
      <div className="flex items-center gap-3 sm:gap-4">
        <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#3a4047] bg-[#11171d] shadow-[0_8px_20px_rgba(0,0,0,0.28)] transition-transform group-hover:scale-110 sm:h-11 sm:w-11">
          {icon}
        </span>
        <h3 className="text-lg font-semibold leading-tight text-[#e1c19a] sm:text-3xl">{tab}</h3>
      </div>
