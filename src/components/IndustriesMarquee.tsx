 ...
 *  what can be read at a glance. There's no per-sector page to send any one
 *  tile to individually, so every tile links to the same place: the full
 *  industries listing. */
function IndustryTile({ tile, href, hidden }: { tile: Tile; href: string; hidden?: boolean }) {
  return (
    <li className="shrink-0">
      <a
        href={href}
        // The duplicate pass exists only to make the scroll loop seamless —
        // aria-hidden on its parent already keeps screen readers off it, and
        // this keeps keyboard Tab from landing on the same link twice.
      ...
      >
        <h3 className="text-base font-semibold leading-snug text-[#e1c19a] transition-colors group-hover:text-[#f4d3ab] sm:text-lg">
          {tile.title}
        </h3>
...
function TilePass({ tiles, href, hidden }: { tiles: Tile[]; href: string; hidden?: boolean }) {
  return (
    <ul
    ...
    >
      {tiles.map((tile) => (
        <IndustryTile key={hidden ? `dup-${tile.key}` : tile.key} tile={tile} href={href} hidden={hidden} />
      ))}
    </ul>
  );
}
...
export default async function IndustriesMarquee() {
  ...
  const industriesHref = localeHref(locale, "/industries");
  ...
  return (
    ...
    >
      ...
      >
        <TilePass tiles={tiles} href={industriesHref} />
        <TilePass tiles={tiles} href={industriesHref} hidden />
      </div>
    </div>
  );
}
